...
let is_none m = match m with None -> true | Some _ -> false
...
let _ =
  ...
  in
  let matmultyp =
    ( [ ("A", Nparray [ Id a; Id b ]); ("B", Nparray [ Id b; Id c ]) ],
      Nparray [ Id a; Id c ] )
  in

  (* good matrix multiplication *)
  let result_typ =
    check_app matmultyp [ Dimensions [ x; y ]; Dimensions [ y; z ] ]
  in
  let _ =
  ...
  let _ =
    try
      let _result_typ =
        check_app matmultyp [ Dimensions [ x; z ]; Dimensions [ y; z ] ]
      in
      assert false
...
let _ =
  ...
  let bad_typ =
    ([ ("X", Nparray [ Spread a ]); (a, Nparray []) ], Nparray [])
  in
  let _ = assert_bad_kind bad_typ in
  ...
  let bad_typ =
    ( [ ("X", Nparray [ Spread a; Spread b; Add (Spread a, Spread b) ]) ],
      Nparray [] )
  in
  let _ = assert_bad_kind bad_typ in
  ...
  let bad_typ =
    ( [ ("X", Nparray [ Spread a; Spread b; Mul (Spread a, Spread b) ]) ],
      Nparray [] )
  in
  let _ = assert_bad_kind bad_typ in
  ...
  let bad_typ =
    ( [ (a, Nparray [ Spread b ]); (c, Nparray [ Id d ]) ],
      Nparray [ Drop (b, [ Left c ]) ] )
  in
  let _ = assert_bad_kind bad_typ in
...
let _ =
  ...
  let add_typ =
    ([ ("A", Nparray [ Id a; Id b ]) ], Nparray [ Add (Id a, Id b) ])
  in
  match check_app add_typ [ Dimensions [ x; y ] ] with
  | Dimensions [ x' ] -> assert (prove_int_eq (mk_int x') (add_int [ x; y ]))
  | _ -> assert false
...
let _ =
  ...
  in
  let nested_Add_typ =
    ( [ ("X", Nparray [ Id a; Id b; Id c ]) ],
      Nparray [ Add (Id a, Add (Id b, Id c)) ] )
  in
  match check_app nested_Add_typ [ Dimensions [ x; y; z ] ] with
  | Dimensions [ x' ] -> assert (prove_int_eq (mk_int x') (add_int [ x; y; z ]))
  | _ -> assert false
...
let _ =
  let a, b, x, y = (mk_string (), mk_string (), mk_string (), mk_string ()) in
  let mul_typ =
    ([ ("X", Nparray [ Id a; Id b ]) ], Nparray [ Mul (Id a, Id b) ])
  in
  match check_app mul_typ [ Dimensions [ x; y ] ] with
  | Dimensions [ x' ] -> assert (prove_int_eq (mk_int x') (mul_int [ x; y ]))
  | _ -> assert false
...
let _ =
  ...
  let spread_typ =
    ( [ ("X", Nparray [ Spread a ]); ("Y", Nparray [ Spread a ]) ],
      Nparray [ Spread a ] )
  in
  let _ =
  ...
  let spread_typ =
    ( [ ("X", Nparray [ Spread a; Id b ]); ("Y", Nparray [ Id b ]) ],
      Nparray [ Id b; Spread a ] )
  in
  let _ =
  ...
  let spread_type =
    ( [
        ("X", Nparray [ Spread a; Id b; Spread c ]);
        ("Y", Nparray [ Spread a; Spread c ]);
      ],
      Nparray [ Spread c ] )
  in
  let _ =
    match
      check_app spread_type [ Dimensions [ x; y; z ]; Dimensions [ x; z ] ]
    with
...
let _ =
  ...
  let drop_typ =
    ( [ (a, Nparray [ Spread b ]); (c, TypeInt); (d, TypeInt) ],
      Nparray [ Drop (b, [ Left c; Left d ]) ] )
  in
  let _ =
    match
      check_app drop_typ
        [ Dimensions [ d; e; f; g ]; LiteralInt 0; LiteralInt 1 ]
    with
  ...
  let _ =
    match
      check_app drop_typ
        [ Dimensions [ d; e; f; g ]; LiteralInt 0; Dimensions [ e; f; g ] ]
    with
  ...
  let _ =
    ...
    match
      check_app drop_typ [ Dimensions [ d; e; f; g ]; LiteralInt 0; res ]
    with
    | Dimensions l -> prove_list_eq l [ e; f; g ]
    | _ -> assert false
  in
  ...
  let _ =
    try
      let _ =
        check_app drop_typ_simp [ Dimensions [ d; e; f ]; LiteralInt 3 ]
      in
      assert false
    with TypeError _ -> assert true
  in
  ...
  let drop_typ_const =
    ([ (a, Nparray [ Spread b ]) ], Nparray [ Drop (b, [ Right 0 ]) ])
  in
  let _ =
  ...
  let drop_neg =
    ([ (a, Nparray [ Spread b ]) ], Nparray [ Drop (b, [ Right ~-1 ]) ])
  in
  let _ =
...
let _ =
  ...
  let _ =
    match
      check_app keep_typ
        [ Dimensions [ d; e; f; g ]; LiteralInt 0; Dimensions [ d ] ]
    with
  ...
  let _ =
    ...
    match
      check_app keep_typ [ Dimensions [ d; e; f; g ]; LiteralInt 0; res ]
    with
    | Dimensions l -> prove_list_eq l [ d ]
    | _ -> assert false
  in
  ...
  let _ =
    try
      let _ =
        check_app keep_typ_simp [ Dimensions [ d; e; f ]; LiteralInt 3 ]
      in
      assert false
    with TypeError _ -> assert true
  in
  ...
  let keep_const_typ =
    ([ (a, Nparray [ Spread b ]) ], Nparray [ Keep (b, [ Right 1 ]) ])
  in
  let _ =
...
let _ =
  ...
  in
  let broadcast_typ =
    ([ (a, Nparray [ Spread b ]); (c, Nparray [ Broadcast b ]) ], Nparray [])
  in
  let _ =
    assert (
      check_app broadcast_typ [ Dimensions [ a; b ]; Dimensions [ b ] ]
      = Dimensions [])
  in

  let _ =
    assert (
      check_app broadcast_typ
        [ Dimensions [ a; b ]; Dimensions [ mk_int_var 1 ] ]
      = Dimensions [])
  in
